  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        {loading ? (
          <div className="text-center py-20 text-gray-400">{t("shop.loading")}</div>
        ) : products.length === 0 ? (
          <div className="text-center py-20 text-gray-400">
            <Icon icon={IconSearch} size={40} className="mx-auto mb-3 opacity-30" />
            <p className="text-sm">{t("shop.noProducts", { suffix: search ? ` para "${search}"` : "" })}</p>
          </div>
        ) : (
        ...
        {totalPages > 1 && (
          <div className="flex justify-center gap-2 mt-8">
            ...
            >
              <Icon icon={IconArrowLeft} size={14} /> {t("shop.prev")}
            </button>
            ...
            <button
              ...
              className="px-4 py-2 rounded-lg border text-sm disabled:opacity-40 hover:bg-gray-100 flex items-center gap-1.5"
            >
              {t("shop.next")} <Icon icon={IconArrowRight} size={14} />
            </button>
          </div>
        )}
      </div>
