...
export default function CartDrawer({ open, onClose }: Props) {
  ...
  return (
    <>
      ...
      <div className={`fixed top-0 right-0 h-full w-full max-w-sm bg-slate-50 dark:bg-slate-800 shadow-2xl z-50 flex flex-col transform transition-transform duration-300 ${
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
          ...
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-md p-1 transition-colors dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-slate-700"
          >
        ...
        <div className="flex-1 overflow-y-auto">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-slate-400 dark:text-slate-400 gap-3 px-4">
              ...
              <p className="text-xs text-slate-400 dark:text-slate-400 text-center">{t("cart.emptyDrawerDesc")}</p>
            </div>
          ) : (
              ...
              {items.map((item) => (
                <div key={item.productId} className="flex items-center gap-3 py-4">
                  <img
                    src={item.imageUrl}
                    alt={item.name}
                    className="w-14 h-14 rounded-xl object-cover flex-shrink-0 bg-slate-100 dark:bg-slate-700"
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-2">
                      <button
                        ...
                        className="w-6 h-6 rounded-lg border border-slate-200 bg-white text-slate-600 hover:bg-slate-100 text-sm font-bold flex items-center justify-center transition-colors dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600"
                      >−</button>
                      <span className="text-xs font-semibold w-8 text-center text-slate-700 dark:text-slate-300">
        ...
        {/* Footer */}
        {items.length > 0 && (
          <div className="border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-5 py-4 space-y-3">
            <div className="flex justify-between items-baseline">
              <span className="text-sm text-slate-500 dark:text-slate-400">{t("cart.subtotal")}</span>
